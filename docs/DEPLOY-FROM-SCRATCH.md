...
8. При наличии платной подписки: использовать пункт «Нативный логин» для OAuth-авторизации (Qwen OAuth, Claude подписка, OpenCode providers). Там же есть подпункт «Запуск … (ванильный запуск)» для старта `qwen` / `claude` / `opencode` без пресетов.
