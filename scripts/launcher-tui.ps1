...
# ─── Invoke-ChildCliSafe ─────────────────────────────────────────────────────
# Wrapper around '& $exe' that survives Ctrl+C / non-zero exit codes so the
# outer TUI launcher loop can return to the main menu instead of terminating.
#
# Why this exists:
# - In PS 7.4+ $PSNativeCommandUseErrorActionPreference defaults to $true.
#   Combined with $ErrorActionPreference = "Stop" (which launchers use), any
#   native command that exits with a non-zero code is converted into a
#   terminating error and aborts the whole script.
# - Claude Code (claude.exe), OpenClaude (openclaude.exe), Freebuff
# (freebuff.exe) and OpenCode (opencode.exe) return non-zero on Ctrl+C
#   (typically STATUS_CONTROL_C_EXIT = 0xC000013A = -1073741510).
# - Qwen Code (Node.js via qwen-code.cmd) returns 0 on Ctrl+C, which is why
#   the Qwen launcher doesn't need this wrapper.
#
# This function temporarily relaxes both settings, runs the child CLI in a
# try/catch, and always returns $LASTEXITCODE to the caller.
function Invoke-ChildCliSafe {
  param(
    [Parameter(Mandatory = $true)][string]$ExePath,
    [string[]]$Arguments = @()
  )
  $prevEap = [System.Management.Automation.ActionPreference]$ErrorActionPreference
  ...
  try {
    ...
    # PipelineStoppedException or similar from Ctrl+C — swallow, return non-zero
    return -1
  } finally {
