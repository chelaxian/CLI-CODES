...
function Invoke-CliCommand {
  ...
  # Delegate to the safe wrapper from launcher-tui.ps1 so Ctrl+C / non-zero
  # exits don't terminate the launcher — we'll return to the main menu.
...
function Invoke-OpenCodeProfile {
  ...
  switch ($ProfileId) {
    "zai-glm" {
      ...
      $null = Invoke-ChildCliSafe -ExePath $opencodeExe
      return
    }
    "zai-glm51" {
      $apiKey = [Environment]::GetEnvironmentVariable("ZAI_API_KEY", "User")
